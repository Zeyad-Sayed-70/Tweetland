...
const NavigationSide = () => {
  ...
  return (
    <NavigationProvider>
      <a href="/" style={{ width: "fit-content", display: "block" }}>
        <Box className="logo-box" mb={3}>
          <img style={{ width: "40px" }} src="assets/twitter-logo.png" />
        </Box>
      </a>
      ...
      <Stack className="ul" gap={1}>
        {navigationList.map((list, ind) => {
          if (
            (!isLogging &&
              (ind <= 1 || ind === navigationList.length - 1 || ind === 4)) ||
            isLogging
          )
            return (
              <Box
              ...
              >
                ...
                <Typography
                  ...
                  fontWeight={600}
                >
                  {t(`commons.navigation.lists.${list.title}`)}
                </Typography>
              </Box>
            );
        })}
