...
const Interactions = ({ data }) => {
  ...
  const {
    ...
    isLoading,
    isSuccess,
  } = useSelector((state: RootState) => state.tlike);
  ...
  useEffect(() => {
    ...
    const isLike = data?.interactions?.likes.users.some(
      (user) => user === me?._id
    );
    const isRetweet = data?.interactions?.retweets.users.some(
      (user) => user === me?._id
    );
  ...
  const onLike = () => {
    ...
    if (inAct.isLike) {
      setTweetData({
        ...tweetData,
        interactions: {
          ...tweetData.interactions,
          likes: {
            ...tweetData.interactions.likes,
            count: parseInt(tweetData.interactions.likes.count) - 1,
          },
        },
      });
    } else {
  ...
  const onRetweet = () => {
    ...
    formData.append(
      "content",
      JSON.stringify({ text: data.content.text, url: data.content?.url })
    );
    formData.append(
      "creator_data",
      JSON.stringify({
    ...
    setTweetData({
      ...
      interactions: {
        ...tweetData.interactions,
        retweets: {
          ...tweetData.interactions.retweets,
          count: parseInt(tweetData.interactions.retweets.count) + 1,
        },
      },
    });
    ...
    <>
      ...
      >
        <Stack direction="row" alignItems="center" gap={0.5}>
          <Tooltip title={t("commons.tweet.interactions.title.comment")}>
            <IconButton disabled={!isLogging} id="comment">
              <ChatBubbleOutlineOutlinedIcon />
            </IconButton>
          </Tooltip>
          <Typography fontWeight={600}>
            {tweetData?.interactions?.comments}
          </Typography>
        </Stack>
        <Stack
          className={`${inAct.isRetweet ? "active" : ""}`}
          direction="row"
          alignItems="center"
          gap={0.5}
        >
          <Tooltip title={t("commons.tweet.interactions.title.retweet")}>
            <IconButton
              disabled={!isLogging}
              id="repost"
          ...
          <Typography fontWeight={600}>
            {tweetData?.interactions?.retweets.count}
          </Typography>
          <AccseptDialog
            open={open}
            setOpen={setOpen}
        ...
        <Stack
          className={`${inAct.isLike ? "active" : ""}`}
          direction="row"
          alignItems="center"
          gap={0.5}
        >
          <Tooltip title={t("commons.tweet.interactions.title.like")}>
            <IconButton disabled={!isLogging} id="like" onClick={onLike}>
              <FavoriteBorderOutlinedIcon />
            </IconButton>
          </Tooltip>
          <Typography fontWeight={600}>
            {tweetData?.interactions?.likes.count}
          </Typography>
        </Stack>
        <Stack direction="row" alignItems="center" gap={0.5}>
          <Tooltip title={t("commons.tweet.interactions.title.share")}>
            <IconButton disabled={!isLogging} id="share">
