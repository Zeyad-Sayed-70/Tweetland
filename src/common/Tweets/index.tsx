...
const Tweet = ({data}) => {
  ...
  const { response, isLoading, isSuccess } = useSelector((state: RootState) => state.utweet);
  ...
  return (
    <TweetsProvider ref={tweetRef}>
      ...
      <Stack gap={1} flex={1}>
        ...
        {data.type === "text" && <Typography contentEditable={edit} onInput={(e) => setEditedText((e.target as HTMLElement).innerText)}>
