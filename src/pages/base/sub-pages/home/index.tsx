...
const Home = () => {
  ...
  const { tweets: { tweets }, isLoading } = useSelector((state: RootState) => state.gtweet);
  ...
  return (
    <HomeProvider>
      <Stack className='main-header' direction='row' alignItems='center' justifyContent='space-between' sx={{ p: 2 }}>
        <Typography fontSize={20} fontWeight={600} textTransform='capitalize'>{t('pages.home.header')}</Typography>
        <Icon><AutoAwesomeOutlinedIcon sx={{ color: '#179cf0' }}/></Icon>
      </Stack>
     ...
     {isLogging && <CreateTweet />}
      ...
      {(tweets?.length === 0 || !isLoading) && isLogging ? 
      <Typography className='notfound'>{t('pages.home.notFoundMsg')}</Typography> : 
      <Typography className='notfound'>{t('pages.home.notAuthorized')}</Typography>
      }
    </HomeProvider>
  )
}
